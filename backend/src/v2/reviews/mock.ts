...
import ReviewsServiceClass, { ReviewNotFoundError, mkCreateReviews } from './service';
...
export const createReviews = mkCreateReviews({ reviews: jipDataSource.getRepository(Reviews) });
...
export const reviews = s.router(contract.reviews, {
  ...
  patch: {
    ...
    handler: async ({ params: { reviewsId }, req: { user } }) => {
      ...
      return match(result)
        .with(
          P.instanceOf(ReviewNotFoundError),
          () =>
            ({
              status: 404,
              body: { code: 'REVIEWS_NOT_FOUND', message: '검색한 리뷰가 존재하지 않습니다.' },
            } as const),
        )
