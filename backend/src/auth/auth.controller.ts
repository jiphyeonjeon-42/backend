...
export const getToken = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (clientURL === false || !req.user)
      throw new FtError(403, "invalid data");
    const { id } = req.user as any;
    let user: models.User[] = await usersService.searchUserByIntraId(id);
    if (user.length === 0)
      throw new FtError(401, "not found user");
...
export const getMe = async (req: Request, res: Response) => {
  try {
    const { id } = req.user as any;
    let user: { items: models.User[] } = await usersService.searchUserById(id);
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (!id || !password)
      throw new FtError(400, "empty field");
    /* 여기에 id, password의 유효성 검증 한번 더 할 수도 있음 */
    let user: { items: models.User[] } = await usersService.searchUserByEmail(id);
    if (user.items.length === 0)
      throw new FtError(401, "not found user");
    if (!bcrypt.compareSync(password, user.items[0].password))
      throw new FtError(403, "not equals password");
